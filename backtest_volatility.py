...
def run_backtest(data_path, ticker=None, initial_capital=100000, lookback_days=90, start_date=None, end_date=None, 
                ...
                volatility_target=0.02, volatility_lookback=14, max_leverage=4.0, min_leverage=0.5, check_interval_minutes=30, 
    ...
    Parameters:
        ...
        volatility_target: Target daily volatility for position sizing (default: 0.02 = 2%)
        volatility_lookback: Number of days to use for volatility calculation (default: 14)
        max_leverage: Maximum leverage allowed (default: 4.0)
        min_leverage: Minimum leverage to use regardless of volatility (default: 0.5)
    ...
    print(f"\n使用波动率动态仓位控制，目标波动率为 {volatility_target*100}%，最大杠杆为 {max_leverage}倍，最小杠杆为 {min_leverage}倍")
    ...
    # Calculate rolling volatility metrics for position sizing
    daily_prices['rolling_mean'] = daily_prices['daily_return'].rolling(window=volatility_lookback).mean()
    daily_prices['rolling_std'] = daily_prices['daily_return'].rolling(window=volatility_lookback).std()
    
    # 确保波动率值不会过高导致仓位过低
    # 计算年化波动率
    daily_prices['annualized_vol'] = daily_prices['rolling_std'] * np.sqrt(252)
    
    # 如果波动率太低，使用合理的最小值替代
    # 对于TQQQ这样的杠杆ETF，不应该低于20%
    min_annualized_vol = 0.20  # 20% 最小年化波动率
    daily_prices['annualized_vol'] = daily_prices['annualized_vol'].clip(lower=min_annualized_vol)
    
    # Calculate position factor based on target volatility / actual volatility
    # Formula: min(max_leverage, σtarget/σ_ticker,t)
    daily_prices['position_factor'] = volatility_target / daily_prices['annualized_vol']
    
    # 应用杠杆限制
    daily_prices['position_factor'] = daily_prices['position_factor'].clip(lower=min_leverage, upper=max_leverage)
    
    # Fill NaN values with min_leverage (default fallback position size)
    daily_prices['position_factor'] = daily_prices['position_factor'].fillna(min_leverage)
    ...
    print(f"\n波动率动态仓位统计:")
    print(f"平均仓位因子: {position_factors.mean():.2f}倍")
    print(f"中位数仓位因子: {position_factors.median():.2f}倍")
    print(f"最小仓位因子: {position_factors.min():.2f}倍")
    print(f"最大仓位因子: {position_factors.max():.2f}倍")
    
    # Print distribution of position factors
    bins = [0, 0.5, 1, 1.5, 2, float('inf')]
    bin_labels = ['0-0.5倍', '0.5-1倍', '1-1.5倍', '1.5-2倍', '>2倍']
    ...
    for label, count in position_factor_counts.items():
        print(f"  {label}: {count} 天 ({count/len(position_factors)*100:.1f}%)")
    ...
    for i, trade_date in enumerate(unique_dates):
        ...
        # Calculate position size with volatility-based position sizing
        ...
        if trades and print_daily_trades:
            ...
            print(f"{date_str} | 仓位因子: {position_factor:.2f}x | 交易数: {len(trades)} | 总盈亏: ${day_total_pnl:.2f} | {trade_info}")
        ...
        for trade in trades:
            ...
            trade['position_factor'] = position_factor
    ...
    print(f"Strategy: {strategy_name} with Volatility-based Position Sizing")
    print(f"目标波动率: {volatility_target*100:.1f}%, 最大杠杆: {max_leverage:.1f}x, 最小杠杆: {min_leverage:.1f}x")
    
    # Calculate average position factor
    avg_position = daily_df['position_factor'].mean()
    max_position = daily_df['position_factor'].max()
    min_position = daily_df['position_factor'].min()
    print(f"平均仓位因子: {avg_position:.2f}x, 最大仓位因子: {max_position:.2f}x, 最小仓位因子: {min_position:.2f}x")
...
def plot_specific_days(data_path, dates_to_plot, lookback_days=90, plots_dir='trading_plots', 
                      volatility_target=0.02, volatility_lookback=14, max_leverage=4.0, min_leverage=0.5,
    ...
    参数:
        ...
        volatility_lookback: 计算波动率使用的历史天数
...
if __name__ == "__main__":  
    ...
    daily_results, monthly_results, trades, metrics = run_backtest(
        ...
        volatility_target=1.5,            # 目标波动率 8%
        volatility_lookback=14,            # 波动率计算回溯天数
        max_leverage=2.0,                  # 最大杠杆倍数
        min_leverage=0.5,                  # 最小杠杆倍数，确保至少有80%资金使用
